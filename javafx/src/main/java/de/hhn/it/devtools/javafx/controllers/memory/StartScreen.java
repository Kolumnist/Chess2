...
public class StartScreen implements Initializable {
  ...
  void handle(MouseEvent event) {
    ...
    if (source == finishButton) {
      ...
      if (event.getEventType() == MouseEvent.MOUSE_ENTERED) {
        startButton.setStyle("-fx-background-color: LIGHTGRAY; -fx-border-color: BLACK; -fx-border-radius: 10; -fx-background-radius: 10; -fx-border-width: 3;");
      } else if (event.getEventType() == MouseEvent.MOUSE_EXITED) {
        startButton.setStyle("-fx-background-color: WHITE; -fx-border-color: BLACK; -fx-border-radius: 10; -fx-background-radius: 10; -fx-border-width: 3;");
      }
    }
  }
  ...
  void onStartButtonClicked(ActionEvent event) {
    try {
      memoryService.newGame((Difficulty) MemoryAttributeStore.getReference().getAttribute(MemoryServiceController.DIFFICULTY));
